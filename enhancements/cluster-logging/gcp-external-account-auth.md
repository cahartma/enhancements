...
* As an OpenShift administrator, I want the ClusterLogForwarder to authenticate with GCP using Workload 
Identity Federation, removing the need for long-lived service account keys. 
* As an engineer, I want the ClusterLogForwarder to detect the correct authentication method based on 
the credentials file format, utilizing the correct credentials chain form the sdk, and ensuring compatibility with 
both service_account and external_account (WIF) credentials files.
...
1. Pod gets the Bound Service Account Token
   * This is the projected service account token, and is created by OpenShift and 
   mapped to the service account assigned to the pod.
...
This change involves configuration of authentication and is high-risk, in particular since there is 
no currently supported rust sdk.   The current library in use is 'goauth' and it needs to be updated.

The mitigation is to get the patch approved in the upstream community, and provide clear 
validation checks and logs for troubleshooting any potential authentication issues.   All GCP outputs 
should be tested to verify against regression.
...
* Confirm logs forwarded to GCP Cloud Logging are sent successfully using the new access token.
  * (formerly named "Stackdriver Logs" and current vector output type "gcp_stackdriver_logs")  
